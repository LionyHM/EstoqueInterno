...
export default function InOrOut(props){
    ...
    function renderForm():object{
        ...
        arrForm.push(
            <form key={1000} className="w-full py-4 2xl:px-60">
                     <select  className="hidden dark:bg-gray-500 lg:mb-4 display_out_in appearance-none w-32
                 ...
                 <div className="display_register_product w-full h-full hidden flex justify-center">
                     ...
                     <div className="mr-10 inline-block relative w-full">
                         <label className="dark:text-white  block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="UnidadeP">
                             Unidade:
                         </label>
                         <select className="dark:text-white dark:bg-gray-500 unidade_select block appearance-none w-80 bg-gray-100 text-gray-700 border border-gray-200 rounded py-1 px-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm">
                              <option>-</option>
                              {renderUnidades(props.unidades)}
                         </select>
                         <label className="mt-2 dark:text-white  block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="UnidadeP">
                             Tipo de Baixa:
                         </label>
                         <select className="dark:text-white dark:bg-gray-500 tp_just block appearance-none w-80 bg-gray-100 text-gray-700 border border-gray-200 rounded py-1 px-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm">
                                <option>-</option>  
                         ...
                         <label className="mt-2 dark:text-white  block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="justif">
                             Justifique:
                         </label>
                         <input className="dark:text-white  dark:bg-gray-500 mr-1 appearance-none inline-block w-full bg-gray-100 text-gray-700 border rounded py-1 px-3 mb-3 leading-tight focus:outline-none focus:bg-white text-sm" id="justif" type="text" placeholder="Justifique à baixa" />
                     </div> 
                 ...
                 <div className='overflow-x-auto h-52 w-full'>
                     <table className='mx-auto max-w-4xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                         <thead className="bg-gray-900">
                             <tr className="text-white text-left">
                                 ...
                                 <th className="font-semibold text-sm uppercase px-4 py-2 text-center"> Tipo de baixa </th>
                                 <th className="font-semibold text-sm uppercase px-4 py-2 text-center"> Justificativa </th>
                     ...
                     <div className="flex justify-center flex-wrap mt-2">
                         <button onClick={()=>recDatabase()} className="dark:bg-white dark:hover:bg-gray-400 dark:text-black bg-gray-700 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-xs" type="button" > 
                             Salvar
                         </button>
                    </div>
             </form>
    ...
    async function getEstoqueLocal(qntd= null){
    ...
    function clearFields(){
        document.querySelector(".quantidade_product").setAttribute('value', '')
        document.querySelector(".unidade_select").setAttribute('value', '-')
        document.querySelector(".lote_product").setAttribute('value', '')
        document.querySelector(".form_fields_out_quantidade").setAttribute('value', '')
        document.querySelector(".lote_product_out").setAttribute('value', '---')
    ...
    async function filterProduct(codigo: number | string){
        ...
        if(elemento.value == "saida"){
            var arrProduct:any = []
            var qntEstoque = await getEstoqueLocal('qntd')
            document.querySelector(".display_register_product").classList.add("hidden")  
    ...
    async function SearchFotCodeOrName(){
        ...
        if(elemento.value == "saida"){
            var qntEstoque = await getEstoqueLocal(Cookies.get("setor"))  
        }    
    ...
    function renderLote(cod){
        document.querySelector(".lote_product_out").innerHTML = 
        `
        <option>---</option>            
        `
        let arrProduct:any = []
        props.qntEstoque?.forEach((product) =>{
    ...
    function contQntdRest(){
        ...
        if(elemento.value == 'saida'){
            ...
            props.qntEstoque?.forEach((prodQnt) =>{
    ...
    function addList(){
        ...
        if(cd_product.value == "" || ds_product.value == "" || mvtoQtd == "" || cd_uni.value == "-"){
     ...
     /*    if(typeMvto.value == "saida"){            
        ...
        <tr class="trCar">
            ...
            <td class="ds_product">${ds_product.value}</td>
            ...
            <td value="" class="tpJustCar">${document.querySelector(".tp_just").getAttribute('value')}</td>
            <td value="" class="justfiCar text-xs">${document.querySelector("#justif").getAttribute('value')}</td>
    ...
    async function recDatabase(){
        ...
        const justfiCar:any = document.querySelectorAll(".justfiCar")
        const tpJustCar:any = document.querySelectorAll(".tpJustCar")
        ...
        const limit:any = document.querySelectorAll(".limitCar")   
            ...
            for(let i = 0; i < cd_product.length; i++){
                ...
                await rec(cd_mvto,cd_product[i]?.textContent,ds_product[i]?.textContent,mvtoTotal[i]?.textContent,cd_uni[i]?.getAttribute("unidade") ,loteCar[i]?.textContent,valid[i]?.textContent,limit[i]?.textContent,tpJustCar[i]?.textContent,justfiCar[i]?.textContent,true)
            }             
    }


    async function rec(cd_mvto,cd_product,ds_product,mvtoTotal,cd_uni,loteCar,valid, limit,pjustfiCar,justfiCar,saida){

        let response = await fetch("/api/set/setJust",{
            method: "POST",
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({     
                    produto: ds_product,
                    ...
                    cateJust: pjustfiCar,
                    justif: justfiCar == undefined ? '' : justfiCar,
                    codJust: cd_product,
                    justCdSetor: Cookies.get("setor"),
